#!/usr/bin/env python3
"""Optimize 3WM pump frequency, pump current, and DC bias for 4–8 GHz."""
...
def main() -> None:
    ...
    signals = np.arange(4.0, 8.01, 0.1)
    pumps = np.arange(10.4, 11.351, 0.05)
    dc_biases = np.array([300, 400, 500, 600, 700, 800], dtype=float) * 1e-6
    pump_currents = np.array([150, 200, 250, 300, 350, 400], dtype=float) * 1e-6
    ...
    for dc_bias in dc_biases:
        for pump_current in pump_currents:
            if dc_bias + pump_current >= analysis.twpa.Istar:
                continue
            # TWPAnalysis owns a validated copy of the input model, so mutate
            # that copy rather than the pre-construction ``twpa`` instance.
            ...
            for pump in pumps:
                ...
                rows.append(row)
                if row["finite"] and (best_row is None or candidate_score(row) > candidate_score(best_row)):
                    best_row = row
                    best_gain = gain.copy()
    ...
    with (output / "focused_current_bias_scan.csv").open("w", newline="") as handle:
    ...
    qualified = [
        row for row in rows
        if row["finite"]
        and row["mean_in_band_gain_db"] >= 15
        and row["fraction_above_17db"] >= 0.8
        and row["ripple_peak_to_peak_db"] <= 3
    ]
    report = {
        "search": {
            "signal_band_GHz": [4.0, 8.0],
            "pump_band_GHz": [10.4, 11.35],
    ...
    (output / "focused_current_bias_scan.json").write_text(json.dumps(report, indent=2) + "\n")
    with (output / "focused_best_gain.csv").open("w", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(["frequency_GHz", "gain_db"])
        writer.writerows(zip(signals, best_gain))

    fig, ax = plt.subplots(figsize=(9, 5))
    ax.plot(signals, best_gain)
    ax.axhline(17, color="C3", linestyle="--", label="17 dB coverage target")
    ax.set(
        xlabel="Signal frequency (GHz)",
        ylabel="Gain (dB)",
        title=(
            f"Best focused scan: pump={best_row['pump_GHz']:.2f} GHz, "
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(output / "focused_best_gain.png", dpi=180)
    plt.close(fig)
