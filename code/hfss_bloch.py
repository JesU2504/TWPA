"""Forward Bloch modes of reciprocal HFSS supercell two-ports.

The nonlinear solver uses periodic propagation and boundary impedance.
Finite-network S21 phase serves only as an eigenvalue-branch guide."""
...
# Ignore roundoff-scale eigenvalue splitting when selecting the passband branch.
PASSBAND_LOG_MAGNITUDE_TOL = 1e-6
...
def extract_bloch_parameters(s: np.ndarray, z0: float = 50.0) -> BlochParameters:
    ...
    for index, matrix in enumerate(reciprocal_abcd):
        ...
        if np.ptp(log_magnitudes) > PASSBAND_LOG_MAGNITUDE_TOL:
            # ABCD maps port 2 to port 1, so the forward decaying mode has |lambda| > 1.
            # Magnitude selection prevents roundoff from switching stopband branches.
...
def apply_bloch_parameters(
    ...
    *,
    ...
    evanescence[valid] = np.interp(target, samples, bloch.attenuation) / cells_per_supercell
